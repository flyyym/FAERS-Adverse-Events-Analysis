...
/* 表1：人口学特征 */
title "Table 1. Demographic Characteristics of Reports";
...
title "Table 2. Top 20 Reported Adverse Events (PT)";
...
title "Table 3. Top 20 Drugs (Primary Suspect)";
proc print data=drug_top20 noobs;
    var DRUGNAME COUNT;
run;

/* 图1：报告数量按年份趋势（条形图） */
title "Figure 1. Number of Reports by Year";
proc sgplot data=DEMO_unique;
    ...
    *每隔2年显示一次标签。fitpolicy=rotate 标签旋转防止重叠
	fillattrs=(color=steelblue)：自定义条形颜色;
run;


/* 图2：年龄组分布（条形图） */
title "Figure 2. Age Group Distribution";
proc sgplot data=DEMO_unique;
